...
describe("Requests for /api/users", ()=>{
  ...
  it("POST Creates a user that exists with same email", async ()=>{
    const res = await request(app)
      .post('/api/users')
      .set('Authorization', `Bearer ${token}`)
      .send({
...
describe("Requests for /api/users/:username", ()=>{
  ...
  //test or it('GET Returns all users')
  it('GET Returns specific user', async ()=>{
    const result = await userService.findLastInsertedUser();
    // console.log("RESULT>>>>", result)

    const res = await request(app).get('/api/users/'+result.username).set('Authorization', `Bearer ${token}`);
  ...
  it('PATCH Updates specific user', async ()=>{
    const result = await userService.findLastInsertedUser();
    // console.log("RESULT>>>>", result)
  ...
  it('DELETE Delete a specific user', async ()=>{
    const result = await userService.findLastInsertedUser();
